...
# Use existing images
backgrImage = pygame.image.load('5-minutes-city.png')
...
for i in range(10):
    row = []
    for j in range(10):
        r = pygame.Rect(i * (screen_width/8), j * (screen_height/9),
                        (screen_width/8), (screen_height/9))
        row.append(r)
...
while True:
    ...
    if ShowCell:
        try:
            ...
            f = min(dx,dy)
            cellImage = pygame.transform.scale(cellImage, (int(1 * imWidth), int(1 * imHeight)))
